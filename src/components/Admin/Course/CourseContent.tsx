...
const CourseContent:FC<Props> = (props) => {
    ...
    const [isCollepsed, setisCollepsed] = useState(
        Array<any[]|boolean>(courseContentData).fill(false))
        console.warn(isCollepsed)
...
const handleCourseSubmit=(e:FormEvent<HTMLFormElement>)=>{

    e.preventDefault();
  }
        //Add Content in previous Section
  const handleNewContent=(items:any)=>{
   ...
   if(courseContentData.length>0){
    ...
    const newContent:courseContentDataProps={
       videoUrl:"",
       title:'',
       description:"",
       ...
       <div className={` w-full bg-slate-800 p-4 ${showSectionInput ? "mt-10" : "mb-0"}`} key={index}>
         {showSectionInput &&(
           ...
           <input type="text" onChange={(e)=>{
               ...
               setcourseContentData(updateData);
           }}  className={`font-Poppins cursor-pointer text-white bg-transparent outline-none ${items.videoSection==='Unitited Section' ?  "w-[170px]":"w-min"}`}  value={items.videoSection}/>
           <BsPencil className='cursor-pointer text-white'/>
           <br/>
            </div>)}
             <div className='flex w-full items-center justify-between my-0'>
                {isCollepsed[index] ? (
                      <>{items.title ? (<p className=' font-sans pt-4 block  text-white '>
                        {index + 1}. {items.title} </p>) : <></>}</>
                    ) : (<div></div>)}
...
}}  className={`${styles.input} !h-min py-2`} value={items.description} placeholder='Video Description' rows={8}  cols={30}/>
<br />
<br />
<br />
{items.link.map((link:any,linkIndex:number)=>{return <div className="mb-3 block" key={linkIndex}>
...
<div className=' pt-5'>
   <p className=' flex items-center text-[18px] text-white cursor-pointer' 
   onClick={()=>handleNewContent(items)}>
