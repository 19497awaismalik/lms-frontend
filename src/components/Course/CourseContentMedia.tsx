...
const CourseContentMedia:FC<Props> = ({activeVideo,setActiveVideo,data,id,user,refetch}) => {
...
const isReviewExist=courseData?.course?.reviews?.find((item:any)=>item.user._id===user._id);
  ...
  return (
    <div className=' w-[95%] 800px:w-[89%]   py-4 m-auto  '>
        ...
        <div className=' w-full p-4 flex items-center justify-between bg-slate-500 bg-opacity-20 backdrop-blur shadow-[bg-slate-700] rounded shadow-inner'>
            ...
            {activeBar ===1 && (
                    <div>
                        {data && data[activeVideo].link.map((item:any,index:number)=>{
                            return (
...
export const QuestionReply:FC<QuestionProps> =({
 ...
 export const CommentItem:FC<CommentItemProps> =({data,activeVideo,item,index,answer,setAnswer,handleAnswerSubmit,questionId,setQuestionId,answerLoading})=>{
        ...
        <div className=' flex mb-2'>
               ...
               {replyactive && questionId===item._id && (
                ...
                {item.questionReplies.map((item:any)=>{
                    ...
                    return(
                        <div className='w-full pt-2 flex 800px:ml-16 text-black dark:text-white' key={index}>
                <Image 
                src={
                item?.user ?item.user?.avatar?.url:avatar}
                width={50} height={50} alt='avatar' className=' rounded-full w-[50px] h-[50px] border border-black dark:border-none object-cover'/>
