...
const Header:FC<Props> = ({ open, setOpen, activeItem, route, setroute }) => {
    ...
    let [socialAuth, { isSuccess }] = useSocialAuthMutation()
    ...
    return (
        <div className=' w-full relative'>
            <div className={`${active ? " !dark:bg-opacity-50  dark:bg-gradient-to-b dark:from-gray-900   dark:to-black !bg-white !bg-opacity-100 fixed left-0 top-0   h-[80px] z-[80] shadow-xl border-b dark:border-[#ffffff1c]  transition duration-500 w-full dark:bg-black"
                ...
                <div className='w-[95%] 800px:w-[97%] m-auto h-full '>
                                ...
                                user ? (
                                    <Link href='/profile' >
                                        <Image src={user.avatar ? user.avatar.url : avatar}
                                            width={30} height={30} priority={true}
                                    ...
                                    : <HiOutlineUserCircle className='800px:block hidden cursor-pointer
                                 text-black dark:text-white' size={27} onClick={() => setOpen(true)} />
                            }
                            <div >
                            </div>
                                ...
                                <div className='fixed w-[70%] h-screen top-0 right-0 z-[9999999999] bg-white dark:text-white dark:bg-slate-900 dark:bg-opacity-90 '>
                                    <NavItem isMobile={true} activeItem={activeItem} />
                                    {
                                        user ? (
                                            ...
                                            : <HiOutlineUserCircle className='block 800px:hidden cursor-pointer text-black  ml-4
