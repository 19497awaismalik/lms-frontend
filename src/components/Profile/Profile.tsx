...
const Profile:FC<ProfileProps> = ({ user }) => {
  ...
  let { data } = useGetUserAllCoursesQuery(undefined, {})
  ...
  useEffect(() => {
    if (data) {
      const filtercourse = user.courses.map((usercourse:any) => data.courses.find((course:any) => course._id === usercourse._id)).filter((course:any)=>course!==undefined);
            ...
            {course.length === 0 && (
              <div className=' justify-center min-h-[50vh] flex items-center '>
                <h1 className='text-center  text-[18px] font-Poppins  text-black dark:text-white'>
                  You do not any purchased course!
                </h1>
              </div>
            )}
          </div>
